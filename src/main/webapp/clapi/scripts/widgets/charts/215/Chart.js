...
//					var _ax = _evaluate(_me, _xmlToJSON(_child), false);
...
//		_chart = new Highcharts.Chart(_configuration, LateLoading);
	...
	function _getSeriesData(_nodeX)
	{
		...
		if(_collect!=null)
		{
			...
			_handler.WantEvent(_id, _collect.GetId(), 'Redraw', 'Redraw', 'always');
...
//			_parts 		= _categories.split('.');
			var _xxx	= _evaluate(_me, _categories, true);
			_parts 		= _xxx.split('.');
		} else
		{
			var _fromObj = _forNode.getAttribute('categories');
			var _xxx	= _evaluate(_me, _fromObj, true);
			_parts 		= _xxx.split('.');
	...
	function _xmlToJSON(_node)
	{
		...
		for(var i=0; i<_node.attributes.length; i++) 
		{
			switch(_node.attributes[i].nodeName)
			{
				...
				/* BOOLEAN values */
				case 'alighTicks': case 'ignorHiddenSeries': case 'inverted': case 'plotShadow':
				...
				case 'stickyTracking': case 'visible': case 'colorByPoint': case 'sliced':
